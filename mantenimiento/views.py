...
# --- IMPORTS A AÑADIR (DE LA APP FLOTA) ---
try:
    from flota.models import (
        ProcesoCarga, ChecklistInspeccion, LlantasInspeccion, 
        LlantaDetalle, Operador
    )
except ImportError:
    print("Error: No se pudieron importar los modelos de la app 'flota'.")
    ProcesoCarga = None 
# -------------------------------------------
...
def es_admin_o_supervisor(user):
    """
    Comprueba si el usuario está en 'Administrador', 'Supervisor' o es superuser.
    """
    if not user.is_authenticated:
        return False
    return (
        user.groups.filter(name='Administrador').exists() or
        user.groups.filter(name='Supervisor').exists() or
        user.is_superuser
    )

class AdminOrSupervisorRequiredMixin(LoginRequiredMixin, UserPassesTestMixin):
    """
    Mixin para vistas que requieren que el usuario sea Admin O Supervisor.
    """
    def test_func(self):
        return es_admin_o_supervisor(self.request.user)
...
class AdminDashboardView(AdminRequiredMixin, ListView):
    ...
    *** VISTA MODIFICADA ***
    ...
    def get_context_data(self, **kwargs):
        ...
        # --- 1. Stats para Tarjetas (MODIFICADAS) ---
        all_tasks = TareaMantenimiento.objects.all()
        context['count_asignadas'] = all_tasks.filter(status=TareaMantenimiento.ESTADO_ASIGNADA).count()
        context['count_en_proceso'] = all_tasks.filter(status=TareaMantenimiento.ESTADO_EN_PROCESO).count()
        context['count_pendiente_cierre'] = all_tasks.filter(status=TareaMantenimiento.ESTADO_PENDIENTE_CIERRE).count() # <-- Nuevo
        context['count_cerradas'] = all_tasks.filter(status=TareaMantenimiento.ESTADO_CERRADA).count() # <-- Nuevo
        # ----------------------------------------
...
class AdminTareaDetalleView(AdminRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if tarea.tipo_mantenimiento == 'PREVENTIVO':
            subtasks_qs = tarea.subtasks_preventivas.all() 
            context['subtasks'] = subtasks_qs 
...
class TecnicoDashboardView(TecnicoRequiredMixin, ListView):
    ...
    *** VISTA MODIFICADA ***
    ...
    def get_queryset(self):
        ...
        # --- MODIFICADO: Excluir nuevos estados finales ---
        qs = TareaMantenimiento.objects.filter(
            tecnico=self.request.user
        ).exclude(
            status__in=[
                TareaMantenimiento.ESTADO_PENDIENTE_CIERRE, 
                TareaMantenimiento.ESTADO_CERRADA, 
                TareaMantenimiento.ESTADO_CANCELADA
            ]
    ...
    def get_context_data(self, **kwargs):
        ...
        context['tareas_pendientes_count'] = all_my_tasks.filter(status=TareaMantenimiento.ESTADO_ASIGNADA).count()
        context['tareas_en_proceso_count'] = all_my_tasks.filter(status=TareaMantenimiento.ESTADO_EN_PROCESO).count()
        
        seven_days_ago = timezone.now() - timedelta(days=7)
        
        # --- MODIFICADO: Incluir ambos estados 'terminados' por el técnico ---
        completadas_recientes_qs = all_my_tasks.filter(
            status__in=[TareaMantenimiento.ESTADO_PENDIENTE_CIERRE, TareaMantenimiento.ESTADO_CERRADA],
            fecha_fin__gte=seven_days_ago
        )
        # ----------------------------------------------------------------
        context['tareas_completadas_recientes_count'] = completadas_recientes_qs.count()

        # --- 2. CÁLCULO DE TIEMPO PROMEDIO (sin cambios) ---
        ...
        # --- 3. LISTA DE UNIDADES PARA EL FILTRO (sin cambios) ---
        context['unidades'] = Unidad.objects.all().order_by('nombre')

        # --- 4. TAREAS COMPLETADAS (MODIFICADO) ---
                ...
                with transaction.atomic():
                    for subtask in subtasks_a_guardar:
                        ...
                        if subtask.id in files_a_subir:
                            ...
                            # Usamos la función del modelo para generar la ruta
...
def finalizar_tarea(request, pk):
    ...
    *** MODIFICADO ***
    ...
    if request.method == 'POST':
        # --- CAMBIOS AQUÍ ---
        tarea.status = TareaMantenimiento.ESTADO_PENDIENTE_CIERRE # <-- Nuevo estado
        tarea.fecha_fin = timezone.now()
        tarea.calcular_tiempo_total() 
        tarea.save()
        messages.success(request, f"Tarea {tarea.id} finalizada. Pasa a 'Pendiente de Cierre' del Administrador.")
        # --------------------
...
# =============================================================
# === NUEVAS VISTAS Y LÓGICA PARA CIERRE / LLEVAR A CARGA ===
# =============================================================

class CierreMantenimientoListView(AdminOrSupervisorRequiredMixin, ListView):
    """
    Muestra al Admin/Encargado las tareas de mantenimiento que 
    los técnicos finalizaron y que están pendientes de cierre.
    """
    model = TareaMantenimiento
    template_name = 'mantenimiento/cierre_mantenimiento_list.html' # <-- Debes crear este template
    context_object_name = 'tareas_pendientes'

    def get_queryset(self):
        # Muestra solo las tareas en el nuevo estado 'PENDIENTE_CIERRE'
        return TareaMantenimiento.objects.filter(
            status=TareaMantenimiento.ESTADO_PENDIENTE_CIERRE
        ).select_related('unidad', 'tecnico').order_by('fecha_fin')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['titulo'] = "Mantenimientos Pendientes de Cierre"
        return context

def _crear_proceso_carga_flota(tarea_mantenimiento, user_admin):
    """
    Toma una TareaMantenimiento y crea el ProcesoCarga 'dummy' 
    en la app 'flota' para inyectarlo en la cola de diésel/urea.
    
    Esta versión NO requiere modificar los modelos de 'flota'.
    """
    if ProcesoCarga is None:
        messages.error("Error: El modelo ProcesoCarga de Flota no está disponible.")
        return None

    try:
        unidad = tarea_mantenimiento.unidad
        
        # 1. Buscar al Operador "SISTEMA" (creado manualmente en admin)
        try:
            operador_sistema = Operador.objects.get(nombre="SISTEMA", apellido="MANTENIMIENTO")
        except Operador.DoesNotExist:
            print("Error CRÍTICO: El Operador 'SISTEMA MANTENIMIENTO' no existe en la BD de flota.")
            return None
            
        with transaction.atomic():
            
            # 2. Crear LlantasInspeccion (dummy)
            llantas_flota = LlantasInspeccion.objects.create(
                unidad=unidad,
                tecnico=user_admin, # El admin/encargado
                km=unidad.km_actual or 0 
            )
            
            # Crear detalles de llanta 'dummy'
            for i in range(1, 7):
                LlantaDetalle.objects.create(
                    inspeccion=llantas_flota, 
                    posicion=f'Posición {i}', 
                    mm=10, marca='SISTEMA', modelo='SISTEMA', 
                    medida='11R22.5', presion=100
                )

            # 3. Crear ChecklistInspeccion (dummy) - La parte "inteligente"
            dummy_checklist_data = {
                'unidad': unidad,
                'operador': operador_sistema,
                'tecnico': user_admin
            }
            
            # Rellenamos programáticamente TODOS los campos 'BIEN'/'MALO' requeridos
            for field in ChecklistInspeccion._meta.get_fields():
                if isinstance(field, models.CharField) and field.choices:
                    if not field.blank and not field.null and not field.has_default():
                        # Asumimos que 'BIEN' es una opción válida en todos los CHOICES
                        dummy_checklist_data[field.name] = 'BIEN'

            checklist_flota = ChecklistInspeccion.objects.create(**dummy_checklist_data)

            # 4. ¡Crear el ProcesoCarga!
            proceso_carga = ProcesoCarga.objects.create(
                unidad=unidad,
                checklist=checklist_flota,
                inspeccion_llantas=llantas_flota,
                tecnico_inicia=user_admin, # El admin/encargado
                status='PENDIENTE' # <-- Lo ponemos directo en la cola de diesel/urea
            )
            
            return proceso_carga

    except Exception as e:
        print(f"Error al crear proceso de carga desde mantenimiento: {e}")
        return None

@login_required
@user_passes_test(es_admin_o_supervisor) # <-- ¡Cambiado!
def procesar_cierre_mantenimiento(request, pk):
    """
    Procesa los botones "Llevar a Carga" o "Desechar" de la
    lista de 'CierreMantenimientoListView'.
    """
    if request.method != 'POST':
        return redirect('mantenimiento:cierre_list')

    tarea = get_object_or_404(TareaMantenimiento, pk=pk, status=TareaMantenimiento.ESTADO_PENDIENTE_CIERRE)
    action = request.POST.get('action')

    if action == 'desechar':
        # --- ACCIÓN 1: DESECHAR (Fácil) ---
        tarea.status = TareaMantenimiento.ESTADO_CERRADA
        tarea.save()
        messages.info(request, f"La tarea de mantenimiento para {tarea.unidad.nombre} ha sido cerrada y archivada.")
    
    elif action == 'cargar':
        # --- ACCIÓN 2: LLEVAR A CARGA (Compleja) ---
        proceso_carga_creado = _crear_proceso_carga_flota(tarea, request.user)
        
        if proceso_carga_creado:
            # Si el puenteo fue exitoso
            tarea.status = TareaMantenimiento.ESTADO_CERRADA
            tarea.save()
            messages.success(request, f"¡Éxito! La unidad {tarea.unidad.nombre} se ha enviado a 'Pendientes de Finalizar Carga' en la app Flota.")
            # Redirigimos al usuario a la app FLOTA
            return redirect('encargado-pendientes-list')
        else:
            # Si el puenteo falló
            messages.error(request, f"Error al enviar {tarea.unidad.nombre} a carga. Revise que el Operador 'SISTEMA MANTENIMIENTO' exista en Flota.")
    
    return redirect('mantenimiento:cierre_list')

# =============================================================
# === FIN DE SECCIÓN DE CIERRE / LLEVAR A CARGA ===
# =============================================================


# --- FUNCIÓN Y VISTA PARA PDF ---
...
@login_required
@user_passes_test(lambda u: es_admin(u) or es_tecnico_o_supervisor(u)) # Admin o Técnico
def generar_pdf_mantenimiento(request, pk):
    """
    Genera un reporte PDF para una tarea de mantenimiento.
    """
    try:
        ...
        return redirect('mantenimiento:admin_dashboard') # Redirigir a admin dashboard

    # Modificado: Permitir reporte en cualquier estado excepto asignada/cancelada
    if tarea.status in ['ASIGNADA', 'CANCELADA']:
         messages.error(request, "Solo se pueden generar reportes de tareas en proceso o finalizadas.")
         if es_admin(request.user):
             return redirect('mantenimiento:admin_tarea_detalle', pk=pk)
